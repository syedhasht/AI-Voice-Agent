...
def _call_gemini_raw(prompt: str) -> Dict[str, Any]:
    """
    Call the configured LLM API (Gemini or Groq) and return a result dict.
    ...
    if provider == "groq":
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning("Groq API rate limit (429) — quota exceeded")
            ...
            logger.error("Groq API timed out during SQL generation (8s)")
            return {"text": None, "rate_limited": False, "error": True}
